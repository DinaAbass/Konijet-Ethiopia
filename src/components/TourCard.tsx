...
export const TourCard = ({ tour }: { tour: Tour }) => {
  ...
  const catNames = tour.categories
    .map(c => {
      const fallback = CATEGORIES.find(x => x.slug === c)?.name || "";
      return t("categories." + c + ".name", fallback);
    })
    .filter(Boolean)
    .join(", ");
  ...
  return (
    <article className="relative rounded-3xl bg-card border border-border shadow-soft overflow-hidden flex flex-col transition-smooth hover:shadow-elevated">
      ...
      <div className="relative">
        <img src={tour.image} alt={t("tours." + tour.slug + ".title", tour.title)} loading="lazy" className="h-44 w-full object-cover" />
        <span
      ...
      <div className="p-4 pt-5 flex-1 flex flex-col gap-2">
        <h3 className="font-display text-base leading-snug text-primary line-clamp-2">{t("tours." + tour.slug + ".title", tour.title)}</h3>
        <div className="flex items-center gap-1 text-xs text-muted-foreground"><Clock className="h-3.5 w-3.5" /> {t("tours." + tour.slug + ".durationLabel", tour.durationLabel)}</div>
        ...
        <div className="text-sm">
          {hasDiscount ? (
            <div className="flex items-baseline gap-2 flex-wrap">
              ...
              <span className="rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold px-2 py-0.5">{t("tour.sale", "SALE")}</span>
        ...
        <Link href={`/tours/${tour.slug}`} className="mt-auto self-end rounded-full border border-border px-4 py-1.5 text-sm font-medium hover:bg-primary hover:text-primary-foreground transition-smooth">
